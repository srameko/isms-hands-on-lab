/*
 * CVSS v4.0 scoring — bundled from the official FIRST reference implementation
 * https://github.com/FIRSTdotorg/cvss-v4-calculator
 *
 * Copyright (c) 2023 FIRST.ORG, Inc., Red Hat, and contributors
 * SPDX-License-Identifier: BSD-2-Clause
 * (see LICENSE-CVSS in this repository for the full license text)
 *
 * Note: this file keeps the reference code as-is (non-strict),
 * exposing a small API at the bottom as window.CVSS40.
 */
...
/* --- small API used by modules/vulnerability.js --- */
window.CVSS40 = {
  BASE_METRICS: ["AV","AC","AT","PR","UI","VC","VI","VA","SC","SI","SA"],
  score: function (base) {
    var defaults = {E:'X',CR:'X',IR:'X',AR:'X',MAV:'X',MAC:'X',MAT:'X',MPR:'X',MUI:'X',MVC:'X',MVI:'X',MVA:'X',MSC:'X',MSI:'X',MSA:'X'};
    var v = Object.assign({}, defaults, base);
  ...
  vectorString: function (base) {
    return 'CVSS:4.0/' + this.BASE_METRICS.map(function (k) { return k + ':' + base[k]; }).join('/');
  }
};
